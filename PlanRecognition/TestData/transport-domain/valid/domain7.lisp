(define (domain transport)
  ...
  (:task root :parameters (?p2 - package ?l - location ?l2 - location ?p - package))

  (:method m-root
    :parameters (?p2 - package ?l - location ?l2 - location ?p - package)
    :task (root ?p ?p2 ?l ?l2)
    :subtasks (and
      (deliver ?p ?l)
      (deliver ?p2 ?l2))
  ...
  (:method m-drive-to-via
    :parameters (?v - vehicle ?l2 - location ?l3 - location)
    :task (get-to ?v ?l3)
    :ordered-subtasks (and
        (get-to ?v ?l2)
        (drive ?v ?l2 ?l3))
